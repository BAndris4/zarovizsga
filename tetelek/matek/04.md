...
A **neminformált keresési stratégiák** sajátossága, hogy kizárólag a probléma definíciójában megadott strukturális információkra támaszkodnak. Nem rendelkeznek semmilyen kiegészítő, tartomány-specifikus (heurisztikus) tudással arról, hogy egy vizsgált állapot milyen messze van a céltól, így csupán a lehetőségek szisztematikus bejárására szorítkoznak.

#### Alapvető fogalmak

- **Fakereső algoritmus:** Az állapottér strukturált és szisztematikus bejárása. Ennek központi lépése a **kiterjesztés**, amelynek során egy már felfedezett állapotból a lehetséges operátorok alkalmazásával legeneráljuk az összes rákövetkező állapotot.
- **Állapot (State):** A vizsgált probléma egy adott pillanatbeli, fizikai vagy logikai konfigurációjának elméleti reprezentációja.
- **Csúcs (Node):** A keresési fa egy eleme; egy adminisztratív adatszerkezet. Fontos, hogy a csúcs nem azonos magával az állapottal! A csúcs a keresés nyilvántartására szolgál, és jellemzően a következőket tárolja:
  - A hozzá tartozó aktuális állapotot
  - A szülőcsúcsra (előzményre) mutató hivatkozást
  - A keresési fában elfoglalt mélységet
  - A gyermekcsúcsokat (leszármazottakat)
  - A kezdőállapottól odáig vezető út halmozott költségét (útköltség)

#### Keresési stratégiák értékelési szempontjai

Az algoritmusok teljesítményének és megbízhatóságának összehasonlítására négy fő mérőszámot alkalmazunk:

- **Teljesség:** Garantálja-e a módszer, hogy megtalálja a célállapotot, feltéve, hogy a vizsgált állapottérben egyáltalán létezik megoldás?
- **Időbonyolultság:** Mennyi ideig tart megtalálni a megoldást? Ezt a futási idő helyett jellemzően a keresés során generált vagy kiterjesztett csúcsok maximális számával fejezzük ki.
- **Tárbonyolultság:** Mennyi memóriára van szükség az algoritmus futtatásához? Ezt a memóriában egyidejűleg kötelezően tárolandó csúcsok maximális száma határozza meg.
- **Optimalitás:** Ha a problémának több megoldása is létezik, garantálja-e a stratégia, hogy a legkisebb útköltségű (legoptimálisabb) megoldást találja meg elsőként?

#### A komplexitásvizsgálat változói

Az idő- és tárbonyolultság matematikai felírásához (pl. aszimptotikus analízishez) az alábbi állandókat használjuk a keresési fában:

- $b$ **(Elágazási tényező / Branching factor):** A keresési fa maximális elágazási faktora (egy csúcsnak legfeljebb ennyi rákövetkezője, azaz gyermeke lehet).
- $d$ **(Megoldás mélysége / Depth):** A legsekélyebb (legkisebb költségű) célállapot mélysége a fában.
- $m$ **(Maximális mélység / Maximum depth):** Az állapottérbeli útvonalak maximális hossza. Ha az állapottér végtelen, vagy a gráfban végtelen körök (hurkok) alakulhatnak ki, akkor $m = \infty$.

#### Szélességi keresés

A keresés szintenként haladva tárja fel a gráfot. Mindig a kiindulóponttól legkisebb távolságra (legkisebb mélységben) lévő, még nem vizsgált csúcsokat terjeszti ki először, mielőtt egy mélyebb szintre lépne.

#### Mélységi keresés

A keresés áganként halad előre. Mindig a gráf legmélyebb, még nem vizsgált csúcsát terjeszti ki, és csak akkor lép vissza az előző csomóponthoz (backtracking), ha az aktuális ágon már nem tud tovább haladni.

![kereses](../../kepek/kereses_animacio.gif)

#### Egyenletes költségű keresés

A szélességi keresés "okosabb" változata, amelyet súlyozott gráfoknál használunk. Nem feltétlenül szintenként halad, hanem egy prioritási sor segítségével mindig azt a még kiterjesztetlen csúcsot választja, amelynek a kiindulóponttól számított összköltsége a legkisebb. Ha a lépésköltségek nem negatívak, garantáltan a legolcsóbb (optimális) utat találja meg.

#### Mélységkorlátozott keresés

A mélységi keresés egy módosított verziója, amely kap egy beépített "védőhálót" egy maximális mélységi korlát formájában. Az algoritmus úgy halad lefelé az ágakon, mint a sima mélységi keresés, de ha eléri a megadott mélységlimitet, akkor nem megy tovább, hanem visszalép. Ez megakadályozza, hogy végtelen fákban vagy körökben ragadjon.

#### Iteratívan mélyülő keresés

A "legjobb mindkét világból" megközelítés: a mélységi keresés memóriahatékonyságát ötvözi a szélességi keresés teljességével. Úgy működik, hogy ismételten futtatja a mélységkorlátozott keresést, de a korlátot minden iterációban egyel növeli (0, 1, 2, 3...). Bár úgy tűnhet, hogy sok munkát végez feleslegesen a felső szintek újra bejárásával, valójában a fák alján lévő hatalmas csomópontszám miatt ez az ismétlődés elhanyagolható, cserébe nagyon kevés memóriát fogyaszt.

![keresesek](../../kepek/nagy_fas_korlatozott_kereses.gif)

#### Az Informálatlan Keresések Összehasonlítása

- $l$: A beállított **mélységkorlát**.
- $C^*$: Az **optimális megoldás** összköltsége.
- $\epsilon$: A **legkisebb élköltség**, ahol $\epsilon > 0$.

| Algoritmus              | Teljes?                                                   | Optimális?                                 | Időbonyolultság                  | Tárbonyolultság                  |
| :---------------------- | :-------------------------------------------------------- | :----------------------------------------- | :------------------------------- | :------------------------------- |
| **Szélességi**          | **Igen** (ha $b$ véges)                                   | **Igen** (ha minden lépés azonos költségű) | $b^{d+1}$                        | $b^{d+1}$                        |
| **Egyenletes költségű** | **Igen** (ha lépésköltség $\ge \epsilon > 0$)             | **Igen**                                   | $b^{\lceil C^*/\epsilon \rceil}$ | $b^{\lceil C^*/\epsilon \rceil}$ |
| **Mélységi**            | **Nem** (végtelen fában beragadhat)                       | **Nem**                                    | $b^m$                            | $b \cdot m$                      |
| **Mélységkorlátozott**  | **Attól függ** (Igen, ha a cél nincs mélyebben, mint $l$) | **Nem**                                    | $b^l$                            | $b \cdot l$                      |
| **Iteratívan mélyülő**  | **Igen** (ha $b$ véges)                                   | **Igen** (ha minden lépés azonos költségű) | $b^d$                            | $b \cdot d$                      |

**Gráfkeresés**: Az algoritmus használ egy úgynevezett felfedezett (explored) listát. Ebbe kerülnek a már meglátogatott csúcsok, így a keresés a későbbiekben már nem foglalkozik velük (nem terjeszti ki őket újra).

---

#### Heurisztikus keresések

**"Legjobbat először" (Best-First Search) keresés alapelve**

Az informált (heurisztikus) keresések minden csúcsra egy kiértékelő függvényt, $f(n)$-t alkalmaznak. Ez a függvény megbecsüli a csúcs állapotának kívánatosságát, és az algoritmus mindig a "legkívánatosabb" (legkisebb $f(n)$ értékű) csúcsot terjeszti ki először.
A felfedezésre váró csúcsokat a memóriában (a peremen / frontier) egy **prioritási sorban** tároljuk.

---

#### 1. Mohó legjobbat először keresés (Greedy Best-First Search)

A mohó algoritmus csak a jövőbe tekint: mindig azt a csúcsot választja, amelyik a becslés szerint a legközelebb van a célhoz.

- **Kiértékelő függvény:** $f(n) = h(n)$
- $h(n)$: Megbecsüli az $n$ csúcs és a legközelebbi célcsúcs közötti legolcsóbb út költségét.

**Tulajdonságai:**

- **Teljesség:** **Nem.** Végtelen terekben vagy ciklusok esetén végtelen hurokba ragadhat (ha nincs megfelelően kezelve a zárt halmaz/explored list).
- **Optimalitás:** **Nem.** Hajlamos a rövidlátásra, és gyakran nem a legolcsóbb utat találja meg.
- **Időbonyolultság:** $O(b^m)$ (ahol $m$ a maximális mélység). Egy jó heurisztika azonban a gyakorlatban ezt drasztikusan csökkentheti.
- **Tárbonyolultság:** $O(b^m)$, mivel minden kiterjesztett csúcsot a memóriában tart.

![moho](../../kepek/moho_reszletes.gif)

---

#### 2. A\* keresés

Az A\* algoritmus egyszerre tekint a múltba és a jövőbe. Nemcsak a céltól való becsült távolságot veszi figyelembe, hanem az indulóponttól idáig megtett út valós költségét is, így elkerüli a "drága" csapdákat.

- **Kiértékelő függvény:** $f(n) = g(n) + h(n)$
- $g(n)$: Az indulóponttól az $n$ csúcsig megtett út **valós költsége**.
- $h(n)$: Az $n$ csúcstól a célig tartó út **becsült költsége**.
- $f(n)$: Az $n$ csúcson keresztül a célba vezető út **becsült teljes költsége**.

Az A\* keresés feltétele az optimális működéshez:
Egy $h(n)$ heurisztika akkor elfogadható, ha **sosem becsüli túl** a cél elérésének költségét.
Képlettel: $h(n) \leq h^*(n)$, ahol $h^*(n)$ az optimális (valós) költség a célig.
Ezen felül $h(n) \geq 0$, és a célcsúcs esetén a becslés pontosan nulla: $h(c\text{\'{e}}l) = 0$.

**Tulajdonságai:**

- **Teljesség:** **Igen** (ha az élek költsége $> \epsilon > 0$, és a csúcsok elágazási tényezője véges).
- **Optimalitás:** **Igen** (ha a heurisztika elfogadható fás keresésnél, vagy konzisztens gráfkeresésnél).
- **Időbonyolultság:** Exponenciális (a futási idő nagyban függ a heurisztika relatív hibájától és a megoldás hosszától).
- **Tárbonyolultság:** $O(b^m)$, vagyis a memóriát gyorsan feléli, mivel minden felfedezett csúcsot tárolnia kell. Ez az A\* legnagyobb gyakorlati korlátja.

![acsillag](../../kepek/astar_reszletes.gif)
